...
const TeacherGroup = ({groupId}) => {
  ...
  useEffect(() => {
    // setLoading(true)
    fetch(`/api/lessonTeacher/${router.query.slug}`)
      .then((res) => res.json())
      .then((data) => {
        setLessons(data)
        console.log(data)
  ...
  const handleSelectAllClick = (event) => {
    if (!event.target.checked) {
      const newSelecteds = lessons.map((lesson) => lesson.lesson_bundle_id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * perView - lessons.length) : 0;
          ...
          >
            ...
            <TableBody>
              ...
              {stableSort(lessons, getComparator(order, orderBy))
                ...
                .map((lesson, index) => {
                  const isItemSelected = isSelected(lesson.lesson_bundle_id);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, lesson.lesson_bundle_id)}
                      role="checkbox"
                      aria-checked={isItemSelected}
                      tabIndex={-1}
                      key={lesson.lesson_bundle_id}
                    ...
                    >
                      ...
                      <TableCell align="right"><Link href={lesson.video}><a target="_blank">Link</a></Link></TableCell>
