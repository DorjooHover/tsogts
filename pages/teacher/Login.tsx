...
export default function Login() {
    ...
    const handleLogin = async(e: { preventDefault: () => void; }) => {
        e.preventDefault();
            try {
                const data = await axios.post(`/api/user/${user.email}`, null, {params: {password: user.password}})
                setAlert(data.data.result)
                data.data ? setView(true) : setView(false)
                setUser((user) => ({...user, message: data.data.message}));
                data.data.result ? router.push(`/teacher/${user.email}`) : null;
                ...
                <div className='sign'>
                    <nav className=" py-3 px-6">
                        <p className="text-right">Бүртгэлгүй юу? 
                            <Link href='/teacher/Signup'><a className="font-semibold cursor-pointer"> Бүртгүүлэх</a></Link>
                        </p>
                    </nav>
