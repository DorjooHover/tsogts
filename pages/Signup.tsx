...
export default function Signup() {
    const [user, setUser] = useState<User>({email: '', password: '', message: '', phone: undefined, name: '', repeat_password: ''})
    const [userDetail, setUserDetail] = useState<UserDetail>({email: 'gray-300', password: 'gray-300', phone: 'gray-300', name: 'gray-300', repeat_password: 'gray-300'})
    ...
    const handleUser = async(e: { preventDefault: () => void; }) => {
        ...
        try {
            if(user.password != '' && checkedPassword && user.email && user.phone && user.name ) {
                const data = await axios.post('/api/user', null, {params: {email: user.email, password: user.password, name: user.name, phone: user.phone, login: 'students'}})
        ...
        <div className="flex">
                ...
                <nav className=" py-3 px-6">
                        <p className="text-right">Бүртгэлтэй юу? 
                            <Link href='/'><a className="font-semibold cursor-pointer"> Нэвтрэх</a></Link>
                        </p>
                    </nav>
